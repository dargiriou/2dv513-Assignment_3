				...
				if(emptyValues(nameField.getText(),surnameField.getText(), memberIDField.getText()) == true)
				{
					if(bookExists(isbnField.getText()))
					{
						if(memberExists(memberIDField.getText()))
						{
							...
							{
								ResultSet rs = conn.createStatement().executeQuery("SELECT copies FROM BOOKS WHERE isbn='"+this.isbnField.getText()+"'");
								int copiesAfterReservation = (rs.getInt(1));
								System.out.println(copiesAfterReservation);						
								
								if(copiesAfterReservation > 0)
								{
									copiesAfterReservation -= copiesAfterReservation;	
								}
